...
describe('pr-creator.ts — createPullRequest()', () => {
  ...
  it('includes Summary section with AI disclaimer in PR body when summary is provided', async () => {
    mockCreate.mockResolvedValueOnce({
      data: { number: 1, html_url: 'https://example.com/pr/1' }
    })
    mockAddLabels.mockResolvedValueOnce(undefined)
    ...
    expect(body).toContain('generated by an AI model')
  ...
  // -- Summary sanitization (security) ----------------------------------------

  it('sanitizes HTML tags in AI summary', async () => {
    mockCreate.mockResolvedValueOnce({
      data: { number: 1, html_url: 'https://example.com/pr/1' }
    })
    mockAddLabels.mockResolvedValueOnce(undefined)
    ...
    await createPullRequest(
      ...
      SAMPLE_METADATA,
      'token',
      'Fixed <script>alert("xss")</script> vulnerability.'
    )

    const body = (mockCreate.mock.calls[0][0] as Record<string, unknown>)
      .body as string
    expect(body).not.toContain('<script>')
    expect(body).toContain('Fixed alert("xss") vulnerability.')
  })

  it('escapes @mentions in AI summary to prevent notification spam', async () => {
    mockCreate.mockResolvedValueOnce({
      data: { number: 1, html_url: 'https://example.com/pr/1' }
    })
    mockAddLabels.mockResolvedValueOnce(undefined)
    ...
    await createPullRequest(
      ...
      SAMPLE_METADATA,
      'token',
      'Credits to @admin for the fix.'
    )

    const body = (mockCreate.mock.calls[0][0] as Record<string, unknown>)
      .body as string
    // The @mention should be broken with a zero-width space
    expect(body).not.toContain('@admin')
    expect(body).toContain('@\u200Badmin')
  })

  it('escapes issue references in AI summary to prevent cross-links', async () => {
    mockCreate.mockResolvedValueOnce({
      data: { number: 1, html_url: 'https://example.com/pr/1' }
    })
    mockAddLabels.mockResolvedValueOnce(undefined)
